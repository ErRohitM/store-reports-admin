...
from typing import TypedDict, Dict, List, Optional, Tuple, Any, Coroutine
...
class StoreReportTimeWindow():
    ...
    async def get_utc_window_bounds(self,
                                    time_window:str) -> Tuple[datetime, datetime]:
        """
    ...
    async def get_store_timezone(self,
                                 store_id: str) -> Optional[StoreTimeZone]:
        """
...
class StoreBusinessAnalyzer:
    ...
    async def main(self,
                   store_id: str,
                   data: TimeWindowData,
                   window: str) -> defaultdict[Any, list]:
        # parse data
        ...
        final_productivity = defaultdict(list)

        total_uptime, total_downtime = 0, 0 # Initially 0

        # Filter by business hours: store_id, day lookups
        # iterate in window days range
        ...
        for day in range(total_days):
            ...
            if business_hours:
                ...
                if clipped_window_starts and clipped_window_ends:
                    ...
                    if window == 'last_hour' :
                        # in minutes
                        final_productivity[window].append(
                            f"Up Time: {round(total_uptime, 2)} Down Time: {round(total_downtime, 2)}"
                        )
                    else:
                        # in hours
                        final_productivity[window].append(
    ...
    def clip_business_hours_to_local_window(self,
                                                  ...
                                                  business_hour_start_time_local: Optional[list],
                                                  business_hour_stop_time_local: Optional[list]) -> tuple[
        ...
        window_starts = start_stop_local_tz.localize(
            combined_window_starts_naive) if not combined_window_starts_naive.tzinfo else (
            combined_window_starts_naive.astimezone(start_stop_local_tz))
        ...
        window_ends = start_stop_local_tz.localize(
            combined_window_ends_naive) if not combined_window_ends_naive.tzinfo else (
            combined_window_ends_naive.astimezone(start_stop_local_tz))
    ...
    async def get_store_polls_status(self,
        ...
        """
        filter store Active polls in UTC timezone
        :param store_id:
...
class InterpolationWindow:
    ...
    """
    def __init__(self,
                 polls: list[dict[str, Any]] | dict[str, Any],
    ...
    def work_flow(self) -> tuple[int, int]:
